...
import {
  BarTrace, initialTestResultsState,
  initialTestSummaryStatisticsState, ScatterTrace,
  StudyDetailsState, TestResultsState,
...
import {
  ...
  initialDataSet, initialDataSetCollection, initialDataSetCollectionState,
...
export const getHasStudyDetailsLoaded = createSelector(
  ...
  (state: StudyDetailsState): boolean =>
...
export const getCurrentTrainingSets = createSelector(
  ...
  (
    ...
    if (
      ...
      console.log(optimizationState, studyDetailsState);
...
export const getCurrentTestSets = createSelector(
  ...
  (
    ...
    if (
      ...
      console.log(benchmarkState, studyDetailsState);
...
export const getTestSummaryStatistics = createSelector(
  ...
  (
    ...
    for (let benchmarkResult of studyDetailsState.benchmarkResults) {
      ...
      const summaryStatistics = benchmarkResult.analysed_result.statistic_entries.filter((value) => {
      ...
      propertyNames.forEach((propertyName, index) => {
        const propertyData = summaryStatistics.filter((value) => {
          return propertyName == toPropertyName(value.property_type, value.n_components);
        });
  ...
  (
    ...
    for (const benchmarkResult of studyDetailsState.benchmarkResults) {
      ...
      for (let propertyName in benchmarkTraces) {
        ...
        traces[propertyName] = traces[propertyName].concat(Object.values(benchmarkTraces[propertyName]))
